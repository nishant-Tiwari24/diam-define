...
const UserProfile = () => {
  ...
  const copydiam = () => {
    navigator.clipboard
      .writeText(localStorage.getItem("diamPublic"))
      .then(() => {
        setCopyu(true);
        setTimeout(() => {
          setCopyu(false);
...

      {
        showQr && (
          <div className="h-full absolute inset-0 flex items-center justify-center backdrop-blur-sm">
            <div className="flex flex-col justify-between p-5 bg-white rounded-md  w-1/4 h-2/4 border-2 border-stone-400">
